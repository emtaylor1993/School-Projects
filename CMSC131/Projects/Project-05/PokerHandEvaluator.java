public class PokerHandEvaluator {
	...
	// Static methods that shows when a hand has two pairs.
		...
		for (int card = 0; card < cards.length; card++) {
			for (int card2 = 0; card2 < cards.length; card2++) {
				...
				if (cards[card].getValue() == cards[card2].getValue()) {
					if (card3 == 0) {
						card3 = cards[card].getValue();
					} else if (card3 != cards[card].getValue()) {
						return true;
					}
				}
			}
	...
	// Static methods that shows when the hand has three of a kind.
	public static boolean hasThreeOfAKind(Card[] cards){
		for (int idx = 0; idx < cards.length - 1; idx++) {
			for (int card1 = idx + 1; card1 < cards.length; card1++) {
				for (int card2 = card1 + 1; card2 < cards.length; card2++) {
					if (cards[card1].getValue() == cards[idx].getValue() && cards[card2].getValue() == cards[card1].getValue()){
						return true;
					}
				}
			}
	...
	public static boolean hasStraight(Card[] cards) {
		...
		for (int index = 1; index <= 9; index++) {
			if (values[index] == true && values[index + 1] == true 
				&& values[index + 2] == true && values[index + 3] == true && values[index + 4] == true){
				
				return true;
			}
		}
		
		if (values[1] == true && values[10] == true && values[11] == true && values[12] == true && values[13]){
			return true;
		}
