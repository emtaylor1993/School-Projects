...
public class Model {
	...
	public void fishExplosions() {
		...
		while(i.hasNext()) {
			...
			if (curr.getSize() > Fish.MAX_FISH_SIZE) {
				...
				int[] newDir = {Fish.UP, Fish.UP, Fish.RIGHT, Fish.RIGHT, Fish.DOWN, Fish.DOWN, Fish.LEFT, Fish.LEFT};
	...
	public static void fishEatPlant(Fish f, Plant p) {
		int food = (Plant.PLANT_BITE_SIZE > p.getSize() ? p.getSize() : Plant.PLANT_BITE_SIZE);
		f.eat(food);
		p.removeBite(food);
	}
	...
	//Returns true if objects are in the same location.
	private boolean samePlace(Locatable thing1, Locatable thing2){
		if ((thing1.getRow() == thing2.getRow()) && (thing1.getCol() == thing2.getCol()))
			return true;
		return false;
	}
}