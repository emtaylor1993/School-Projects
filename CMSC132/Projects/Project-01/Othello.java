...
public class Othello {
	 ...
	 */
	public void setEntry(Piece color, int row, int col) throws NoSuchElementException {
	...
	public void move(int row, int col) {
		...
		if (validMove(color, row, col)) {
			...
			if (rightUpCol != 8) {
				for (int j = 1; (j <= rightUpCol - col) && (j <= row - rightUpRow); j++) {
					board[row - j][col + j] = color;
				}
			}

			if (leftUpCol !=8) {
				for (int j = 1; (j <= row - leftUpCol) && (j >= col - leftUpCol); j++) {
					board[row - j][col - j] = color;
				}
			}

			if (leftDownCol != 8) {
				for (int j = 1; (j <= col - leftDownCol) && (j <= leftDownRow - row); j++) {
					board[row + j][col - j] = color;
				}
			}

			if (rightDownCol != 8) {
				for (int j = 1; (j <= rightDownCol - col) && (j <= rightDownRow - row); j++) {
					board[row + j][col + j] = color;
				}
			}
	...
	public void move(Piece color, int row, int col) {
		if (validMove(color, row, col)) {
			...
			if (leftUpCol != 8) {
				for (int j = 1; (j <= row - leftUpRow) && (j >= col - leftUpCol); j++) {
					board[row - j][col - j] = color;
				}
			}
