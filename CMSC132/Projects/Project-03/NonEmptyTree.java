...
public class NonEmptyTree<K extends Comparable<K>, V> implements Tree<K, V> {
	 ...
	 */
	public NonEmptyTree(K root, V value, Tree<K, V> leftTree2, Tree<K, V> rightTree2) {
		this.root = root;
		this.value = value;
	...
	public String toString() {
		return (leftTree.toString().equals("") ? "": leftTree.toString() + " ")
			+ root.toString() + "=>" + value.toString() + (rightTree.toString().equals("") ? "": " " + rightTree.toString());
	}
	...
	public NonEmptyTree<K, V> clone() {
		NonEmptyTree<K, V> clonedTree = new NonEmptyTree(root, value, leftTree, rightTree);
