...
public class Graph<V extends Comparable<V>> implements Iterable<V> {
	 ...
	 * vertex destination.
	 */
	public void addEdge(V source, V dest, int cost) throws IllegalArgumentException {
	...
	public void removeEdge(V source, V dest) throws NoSuchElementException {
		...
		if (!(graph.contains(source)) || !(graph.contains(dest)) || !(graph.getEdges(source).contains(dest))) {
	 ...
	 */
	public ShortestPath<V> Dijkstra(V source, V dest) throws IllegalArgumentException {
