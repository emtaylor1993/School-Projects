...
public class Names {
	...
	public String getGirlName(int year, int rank) {
		if (!(names.containsKey(year)) || names.get(year).getListGirls().size() < rank) {
			return null;
		} else
	...
	public String getBoyName(int year, int rank) {
		if(!(names.containsKey(year)) || names.get(year).getListBoys().size() < rank) {
			return null;
		} else
	...
	private class threadClass implements Runnable {
		...
		public void run() {
			...
			try {
				...
				while ((inputLine = in.readLine()) != null && count <= numNames) {
