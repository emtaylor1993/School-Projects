...
let get_transition ts s sym =
    ...
    if s = src && sym = letter then
        ...
        false
    in let extract_dest t = match t with (src, letter, dest) -> dest
...
let rec parse_S l = 
    ...
    match t with
    Tok_Union -> (let (a2,l2) = (parse_S n) in (Union (a1, a2), l2))
    | _ -> (a1, l1)				
...
and parse_A l =
    ...
    let (t, n) = lookahead l1 in	
    match t with 
    Tok_Char c -> (let (a2,l2) = (parse_A l1) in (Concat (a1, a2), l2))
...
and parse_C l =
    ...
    | Tok_LParen -> (let (a2,l2) = (parse_S n) in 
